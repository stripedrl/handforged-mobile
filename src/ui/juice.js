...
/**
 * THE NUMBER FORMATTER LIVES IN core/fmt.js NOW, and is re-exported here.
 *
 * It moved down a layer on 2026-08-11 so core/passives.js could reach it: that
 * file is deliberately Phaser-free (the node suite scores real hands through it
 * without a renderer) and importing this module would have dragged DEPTH, the
 * settings store and the sound bus into every unit test. The re-export is what
 * makes that a zero-call-site change — ~40 `import { fmtNum } from
 * '../ui/juice.js'` lines across the scenes keep reading exactly as they did.
 *
 * IMPORTED AND RE-EXPORTED, not `export ... from`: a bare re-export forwards the
 * name to importers WITHOUT binding it in this module's own scope, and half a
 * dozen helpers below (fmtTotal, the payoff tiers, the float builders) call
 * `fmtNum` directly. That distinction cost one red test run.
 */
import { fmtNum, INFINITY_GLYPH } from '../core/fmt.js';

export { fmtNum, INFINITY_GLYPH };
